...
const db = require("../../db");  // Já corrigido
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const userName = email.split('@')[0].charAt(0).toUpperCase() + email.split('@')[0].slice(1).split('.')[0] || 'Usuário';  // Ex: "Teste" de "teste@example.com"
    ...
    res.status(201).json({ message: "Conta criada com sucesso!", token, user: { id: result.lastInsertRowid, name: userName, email } });
  } catch (error) {
